...
static INLINE void _yy256_fill_buffer(__m256i *buff,
                                      __m256i *end,
                                      __m256i zeros) {
  do {
...
void av1_txb_init_levels_skip_avx2(const tran_low_t *const coeff,
  ...
  _yy256_fill_buffer((__m256i *)lvl_top_buf, (__m256i *)lvl_top_buf_end, y_zeros);
  int i = 0;
  uint8_t *ls = levels;
  ...
  if (width == 4) {
    ...
    do {
      ...
      xx_storel_32(ls + stride, x_zeros);
      xx_storeu_128(ls + stride + TX_PAD_LEFT, _mm256_extracti128_si256(res, 1));
      cf += 32;
      ls += stride << 1;
      i += 2;
    } while (i < height);
...
void av1_txb_init_levels_signs_avx2(const tran_low_t *const coeff, const int width,
