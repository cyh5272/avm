...
static void yv12_copy_chroma_planes(const YV12_BUFFER_CONFIG *src,
                                    YV12_BUFFER_CONFIG *dst, int num_planes) {
  if (num_planes > 1) aom_yv12_copy_u(src, dst);
  if (num_planes > 2) aom_yv12_copy_v(src, dst);
}

/*!\brief Copy all planes from 'src' to 'dst', respecting 'num_planes'
 * Note: aom_yv12_copy_frame() does not have the same behavior as
 * aom_yv12_copy_y, aom_yv12_copy_u and aom_yv12_copy_v. So,
 * aom_yv12_copy_frame() cannot be used here.
 */
static void yv12_copy_all_planes(const YV12_BUFFER_CONFIG *src,
                                 YV12_BUFFER_CONFIG *dst, int num_planes) {
  // Note: aom_yv12_copy_frame does not have the same behavior as
  // aom_yv12_copy_y, aom_yv12_copy_u and aom_yv12_copy_v.
  aom_yv12_copy_y(src, dst);
...
static void cdef_restoration_frame(AV1_COMP *cpi, AV1_COMMON *cm,
  ...
  if (use_restoration) {
    ...
    if (av1_use_cnn_encode(cm,
      ...
      double cnn_rdcosts = 0;
      const int y_cnn_indices = av1_num_cnn_indices_for_plane(cm, AOM_PLANE_Y);
      // const int y_cnn_indices = 1;
      for (int y_cnn_index = 0; y_cnn_index < y_cnn_indices; ++y_cnn_index) {
        const int quadtree_cnn[MAX_MB_PLANE] = {
          av1_allow_cnn_for_plane(cm, AOM_PLANE_Y),
          0,  // av1_allow_cnn_for_plane(cm, AOM_PLANE_U),
        ...
        if (y_cnn_index == 0 || (curr_cnn_errors[0] < cnn_errors[0] &&
                                 curr_cnn_rdcosts < cnn_rdcosts)) {
          memcpy(cnn_errors, curr_cnn_errors, sizeof(cnn_errors));
          memcpy(cnn_indices, curr_cnn_indices, sizeof(cnn_indices));
          cnn_rdcosts = curr_cnn_rdcosts;
      ...
      if (av1_allow_cnn_for_plane(cm, AOM_PLANE_U) && num_planes > 1 &&
        ...
        cm->use_cnn[1] = 1;
        cm->cnn_indices[1] = cnn_indices[1];
      ...
      if (av1_allow_cnn_for_plane(cm, AOM_PLANE_V) && num_planes > 2 &&
        ...
        cm->use_cnn[2] = 1;
        cm->cnn_indices[2] = cnn_indices[2];
...
#if CONFIG_LR_FLEX_SYNTAX
  // If superres is used turn off PC_WIENER for since tx_skip values will
  // not be aligned.
