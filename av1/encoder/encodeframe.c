...
#if CONFIG_TIP
  static const int flag_list[EXTREF_FRAME] = {
    0,
    AOM_LAST_FLAG,
    AOM_LAST2_FLAG,
    AOM_LAST3_FLAG,
...
static AOM_INLINE void setup_prune_ref_frame_mask(AV1_COMP *cpi) {
  if ((!cpi->oxcf.ref_frm_cfg.enable_onesided_comp ||
    ...
    cpi->prune_ref_frame_mask =
        (1 << MODE_CTX_REF_FRAMES) - (1 << EXTREF_FRAME);
#else
...
#if CONFIG_TIP
    for (int ref_idx = EXTREF_FRAME; ref_idx < MODE_CTX_REF_FRAMES; ++ref_idx) {
#else
...
#if CONFIG_TIP
    memcpy(cm->lf.ref_deltas, cm->prev_frame->ref_deltas, EXTREF_FRAME);
#else
...
#if CONFIG_TIP
  memcpy(cm->cur_frame->ref_deltas, cm->lf.ref_deltas, EXTREF_FRAME);
#else
