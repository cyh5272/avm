...
typedef struct InterModeSearchState {
  ...
  int_mv single_newmv[NUM_MV_PRECISIONS][MAX_REF_MV_SEARCH][REF_FRAMES];
  int single_newmv_rate[NUM_MV_PRECISIONS][MAX_REF_MV_SEARCH][REF_FRAMES];
  int single_newmv_valid[NUM_MV_PRECISIONS][MAX_REF_MV_SEARCH][REF_FRAMES];
#else
  // Save a set of single_newmv for each checked ref_mv.
...
static int64_t handle_inter_mode(
  ...
  typedef struct {
    ...
    // of reference frames (i, j). Note: indexing with 'j + 1' is due to the
    // fact that 2nd reference can be -1 (NONE_FRAME).
      ...
      if (!cm->show_frame && x->best_pred_mv_sad < INT_MAX) {
        ...
        for (ref_frame = BWDREF_FRAME; ref_frame <= ALTREF_FRAME; ref_frame++) {
          if (cpi->ref_frame_dist_info
                  .ref_relative_dist[ref_frame - LAST_FRAME] < 0)
  ...
  static AOM_INLINE int prune_ref_frame(
      const AV1_COMP *cpi, const MACROBLOCK *x, MV_REFERENCE_FRAME ref_frame) {
    const AV1_COMMON *const cm = &cpi->common;
    ...
    if (prune_ref_by_selective_ref_frame(
            cpi, x, rf, cm->cur_frame->ref_display_order_hint)) {
      return 1;
    }
  ...
  static AOM_INLINE void collect_single_states(
      const FeatureFlags *const features, MACROBLOCK *x,
  ...
  static void tx_search_best_inter_candidates(
    ...
    store_winner_mode_stats(&cpi->common, x, mbmi, NULL, NULL, NULL,
                            THR_INVALID, NULL, bsize, best_rd_so_far,
...
#endif  // IMPROVED_AMVD
#if CONFIG_ADAPTIVE_MVD && CONFIG_JOINT_MVD
      if ((this_mode == JOINT_AMVDNEWMV ||
           this_mode == JOINT_AMVDNEWMV_OPTFLOW) &&
