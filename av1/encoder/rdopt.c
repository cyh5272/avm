...
static int64_t handle_newmv(const AV1_COMP *const cpi, MACROBLOCK *const x,
  ...
  if (is_comp_pred) {
    ...
    if (do_refine_ms) {
      int_mv start_mv;
      start_mv.as_int = args->single_newmv[ref_mv_idx][refs[0]].as_int;
...
#endif
    flex_mv_cost = cost_mv_precision(
        mode_costs, mbmi->max_mv_precision, mbmi->pb_mv_precision, down_ctx
#if SIGNAL_MOST_PROBABLE_PRECISION
        ,
...
static int64_t rd_pick_intrabc_mode_sb(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  // TODO(chiyotsai@google.com): Change the resolution here to MV_SUBPEL_NONE in
  // a separate commit.
#if CONFIG_FLEX_MVRES
...
static bool skip_this_mode_for_this_precision(const uint32_t mode_mask,
                                              const PREDICTION_MODE this_mode) {
  return (mode_mask >> this_mode) & 0x1;
}

static void set_mode_mask_for_this_precision(uint32_t *pmode_mask,
                                             const PREDICTION_MODE this_mode) {
  *pmode_mask |= (1 << this_mode);
}

static void create_mode_mask_for_this_precision(
    uint32_t *pmode_mask, const int64_t *rd_lists,
    const PREDICTION_MODE best_mode, const PREDICTION_MODE second_best_mode) {
  double threshold = 1.10;  // larger value means less complexity reduction
  *pmode_mask = 0;

  // best mode is not valid
  if (rd_lists[best_mode] == INT64_MAX) return;
  int valid_second_best = (rd_lists[second_best_mode] != INT64_MAX);

  for (THR_MODES midx = THR_MODE_START; midx < THR_MODE_END; ++midx) {
    const THR_MODES mode_enum = av1_default_mode_order[midx];
    const MODE_DEFINITION *mode_def = &av1_mode_defs[mode_enum];
    const PREDICTION_MODE this_mode = mode_def->mode;
    if (have_precision_in_inter_mode(this_mode)) {
      if (valid_second_best && this_mode == second_best_mode) continue;
      if (this_mode == best_mode) continue;

      if ((double)rd_lists[this_mode] >
          (threshold * (double)rd_lists[best_mode])) {
        set_mode_mask_for_this_precision(pmode_mask, this_mode);
      }
    }  // if (have_precision_in_inter_mode(this_mode))
  }    // for (THR_MODES midx = THR_MODE_START; midx < THR_MODE_END; ++midx)
}
...
void av1_rd_pick_inter_mode_sb(struct AV1_COMP *cpi,
  ...
  PREDICTION_MODE best_mode[NUM_MV_PRECISIONS] = { MB_MODE_COUNT, MB_MODE_COUNT,
                                                   MB_MODE_COUNT, MB_MODE_COUNT,
                                                   MB_MODE_COUNT, MB_MODE_COUNT,
                                                   MB_MODE_COUNT };
  int64_t best_rd_this_prec[NUM_MV_PRECISIONS] = {
    INT64_MAX, INT64_MAX, INT64_MAX, INT64_MAX, INT64_MAX, INT64_MAX, INT64_MAX
  };

  PREDICTION_MODE second_best_mode[NUM_MV_PRECISIONS] = {
    MB_MODE_COUNT, MB_MODE_COUNT, MB_MODE_COUNT, MB_MODE_COUNT,
    MB_MODE_COUNT, MB_MODE_COUNT, MB_MODE_COUNT
  };
  int64_t second_best_rd_this_prec[NUM_MV_PRECISIONS] = {
    INT64_MAX, INT64_MAX, INT64_MAX, INT64_MAX, INT64_MAX, INT64_MAX, INT64_MAX
  };
  uint32_t mode_bit_mask[NUM_MV_PRECISIONS] = { 0 };
#endif

  for (int pb_mv_precision = mbmi->max_mv_precision;
       pb_mv_precision >= MV_PRECISION_8_PEL; pb_mv_precision--) {
    num_single_modes_processed = 0;
    mbmi->pb_mv_precision = pb_mv_precision;
    intra_mode_num = 0;
#if EANBLE_EARLY_TERMINATION
    best_rd_this_prec[mbmi->pb_mv_precision] = INT64_MAX;
    second_best_rd_this_prec[mbmi->pb_mv_precision] = INT64_MAX;
    int64_t rd_lists[MB_MODE_COUNT];
    mode_bit_mask[mbmi->pb_mv_precision] = 0;
#endif
#endif
    for (THR_MODES midx = THR_MODE_START; midx < THR_MODE_END; ++midx) {
      // Get the actual prediction mode we are trying in this iteration
    ...
    init_mbmi(mbmi, this_mode, ref_frames, cm);
      ...
      if (is_pb_mv_precision_active(cm, mbmi, bsize) &&
...
#if 0
        int valid_best_mode_max_prec = (best_mode[mbmi->max_mv_precision] != MB_MODE_COUNT);
        if (valid_best_mode_max_prec &&
            (!have_precision_in_inter_mode(best_mode[mbmi->max_mv_precision])) )
            continue;
#endif
#if ET_TEST_NUMBER == 1
        ...
        if ((down % 2) == 1) {
          int valid_best_mode =
              (best_mode[mbmi->pb_mv_precision + 1] != MB_MODE_COUNT);
          if (valid_best_mode) {
            if (mbmi->mode != best_mode[mbmi->pb_mv_precision + 1]) continue;
          }
        }
#elif ET_TEST_NUMBER == 2
        int down = mbmi->max_mv_precision - mbmi->pb_mv_precision;
        if (down % 2 == 1) {
          MvSubpelPrecision base_precision = (mbmi->pb_mv_precision + 1);
          int valid_best_mode = (best_mode[base_precision] != MB_MODE_COUNT);
          int valid_second_best_mode =
              (second_best_mode[base_precision] != MB_MODE_COUNT);
          if (valid_best_mode && valid_second_best_mode) {
            if (mbmi->mode != best_mode[base_precision] &&
                mbmi->mode != second_best_mode[base_precision])
              continue;
          } else if (valid_best_mode) {
            if (mbmi->mode != best_mode[base_precision]) continue;
          } else if (valid_second_best_mode) {
            if (mbmi->mode != second_best_mode[base_precision]) continue;
          }
        }
#elif ET_TEST_NUMBER == 3
        if (mbmi->pb_mv_precision != MV_PRECISION_ONE_PEL) {
          MvSubpelPrecision base_precision =
              (mbmi->pb_mv_precision < MV_PRECISION_ONE_PEL)
...
#elif ET_TEST_NUMBER == 4
        if (mbmi->pb_mv_precision != MV_PRECISION_ONE_PEL) {
          MvSubpelPrecision base_precision =
              (mbmi->pb_mv_precision < MV_PRECISION_ONE_PEL)
                  ? MV_PRECISION_ONE_PEL
                  : mbmi->max_mv_precision;
          if (skip_this_mode_for_this_precision(mode_bit_mask[base_precision],
                                                mbmi->mode))
            continue;
        }
#endif
...
#endif
        rd_lists[mbmi->mode] = this_rd;
        if (this_rd < best_rd_this_prec[mbmi->pb_mv_precision]) {
          second_best_rd_this_prec[mbmi->pb_mv_precision] =
              best_rd_this_prec[mbmi->pb_mv_precision];
          second_best_mode[mbmi->pb_mv_precision] =
              best_mode[mbmi->pb_mv_precision];

          best_rd_this_prec[mbmi->pb_mv_precision] = this_rd;
          best_mode[mbmi->pb_mv_precision] = mbmi->mode;
        } else if (this_rd < second_best_rd_this_prec[mbmi->pb_mv_precision] &&
                   this_rd != best_rd_this_prec[mbmi->pb_mv_precision]) {
          second_best_rd_this_prec[mbmi->pb_mv_precision] = this_rd;
          second_best_mode[mbmi->pb_mv_precision] = mbmi->mode;
        }
#if ET_TEST_NUMBER == 3 || ET_TEST_NUMBER == 2 || ET_TEST_NUMBER == 4 || \
    ET_TEST_NUMBER == 1
      }
      ...
      if (this_rd < search_state.best_rd) {
        ...
        error_check_flexmv(
            !(IMPLIES(comp_pred,
                      cm->current_frame.reference_mode != SINGLE_REFERENCE)),
            &cm->error, " Error in ref selection");
#endif
...
#if CONFIG_FLEX_MVRES
#if EANBLE_EARLY_TERMINATION
    assert(second_best_rd_this_prec[mbmi->pb_mv_precision] >=
           best_rd_this_prec[mbmi->pb_mv_precision]);
    create_mode_mask_for_this_precision(
        &mode_bit_mask[mbmi->pb_mv_precision], rd_lists,
        best_mode[mbmi->pb_mv_precision],
        second_best_mode[mbmi->pb_mv_precision]);
    ...
    if (mbmi->pb_mv_precision < mbmi->max_mv_precision) {
      double factor = 1.05;
      if ((double)best_rd_this_prec[mbmi->pb_mv_precision] >
          factor * (double)best_rd_this_prec[mbmi->pb_mv_precision + 1])
        break;
    }
#endif
#endif
