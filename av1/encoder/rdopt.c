...
static int64_t motion_mode_rd(
  ...
  for (int mode_index = mode_index_start; mode_index <= mode_index_end;
    ...
    int is_warpmv_warp_causal =
        warped_causal_idx_map(mode_index) && (base_mbmi.mode == WARPMV);
#endif  // CONFIG_WARPMV
    ...
    if (mbmi->motion_mode == SIMPLE_TRANSLATION && !is_interintra_mode) {
      ...
      } else if (mbmi->motion_mode == WARPED_CAUSAL_INTERINTRA) {
#if CONFIG_EXTENDED_WARP_PREDICTION
...
// Check NEARMV, GLOBALMV ref mvs for duplicate and skip the relevant
// mode
static INLINE int check_repeat_ref_mv(const MB_MODE_INFO_EXT *mbmi_ext,
  ...
  if (single_mode == NEARMV) {
    // when ref_mv_count = 0, NEARMV is same as GLOBALMV
    ...
    // NEARMV, even if they have the same MV. So always try GLOBALMV
