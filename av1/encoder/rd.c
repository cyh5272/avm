...
void av1_fill_coeff_costs(CoeffCosts *coeff_costs, FRAME_CONTEXT *fc,
  ...
  const int plane_type = 0;
  {
    ...
    for (int ctx = 0; ctx < COEFF_BASE_PH_CONTEXTS; ++ctx) {
      av1_cost_tokens_from_cdf(pcost->base_ph[ctx], fc->coeff_base_ph_cdf[ctx],
                               NULL);
    }
    ...
    for (int ctx = 0; ctx < COEFF_BR_PH_CONTEXTS; ++ctx) {
      ...
      for (i = 0; i < COEFF_BASE_RANGE; i += BR_CDF_SIZE - 1) {
        for (j = 0; j < BR_CDF_SIZE - 1; j++) {
          pcost->br_ph[ctx][i + j] = prev_cost + br_ph_rate[j];
        }
        prev_cost += br_ph_rate[j];
      }
      pcost->br_ph[ctx][i] = prev_cost;
    }
    for (int ctx = 0; ctx < COEFF_BR_PH_CONTEXTS; ++ctx) {
      pcost->br_ph[ctx][0 + COEFF_BASE_RANGE + 1] = pcost->br_ph[ctx][0];
      for (int i = 1; i <= COEFF_BASE_RANGE; ++i) {
        pcost->br_ph[ctx][i + COEFF_BASE_RANGE + 1] =
            pcost->br_ph[ctx][i] - pcost->br_ph[ctx][i - 1];
      }
    }
  }
