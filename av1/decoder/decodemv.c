...
#if CONFIG_ADAPTIVE_MVD
      is_adaptive_mvd
          ? aom_read_symbol(r, mvcomp->res_classes_cdf, MV_CLASSES, ACCT_STR)
          :
#endif
...
static INLINE void read_mv(aom_reader *r, MV *mv, const MV *ref,
  ...
  const MV_JOINT_TYPE joint_type =
      ...
      is_adaptive_mvd ? (MV_JOINT_TYPE)aom_read_symbol(
                            r, ctx->restrained_joints_cdf, MV_JOINTS, ACCT_STR)
                      :
#endif
...
static INLINE int assign_mv(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  if (has_second_ref(mbmi)) {
    ...
    same_side = (cur_ref_side > 0 && other_ref_side > 0) ||
...
#endif
#if CONFIG_ADAPTIVE_MVD
  const int is_adaptive_mvd = enable_adaptive_mvd_resolution(cm, mode);
#endif
  switch (mode) {
    case NEWMV: {
    ...
    case JOINT_NEWMV: {
      ...
      read_mv(r, &mv[jmvd_base_ref_list].as_mv,
              &ref_mv[jmvd_base_ref_list].as_mv, 0, nmvc, allow_hp);
#else
      read_mv(r, &mv[jmvd_base_ref_list].as_mv,
              &ref_mv[jmvd_base_ref_list].as_mv, nmvc, allow_hp);
#endif
...
#if CONFIG_JOINT_MVD
    if (compound_ref1_mode(mbmi->mode) == NEWMV || mbmi->mode == JOINT_NEWMV)
#else
