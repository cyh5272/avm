  ...
  *tx_type = DCT_DCT;
  ...
  if (get_ext_tx_types(tx_size, inter_block, cm->features.reduced_tx_set_used) >
    ...
    if (inter_block) {
      ...
      *tx_type = av1_tx_idx_to_type(aom_read_symbol(r, ec_ctx->intra_ext_tx_cdf[eset][square_tx_size][intra_mode], av1_num_ext_tx_set_intra[tx_set_type], ACCT_STR), tx_set_type, intra_mode, size_info);
#else
