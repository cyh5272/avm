...
static AOM_INLINE void decode_block(AV1Decoder *const pbi, ThreadData *const td,
  ...
  decode_token_recon_block(pbi, td, r, partition, bsize);
#if CONFIG_IBC_SR_EXT
  av1_mark_block_as_coded(&td->dcb.xd, mi_row, mi_col, bsize,
                          pbi->common.seq_params.sb_size);
#endif  // CONFIG_IBC_SR_EXT
}
...
static AOM_INLINE void setup_loopfilter(AV1_COMMON *cm,
...
#if CONFIG_TIP
    memcpy(lf->ref_deltas, cm->prev_frame->ref_deltas, EXTREF_FRAME);
#else
...
#if CONFIG_TIP
      const int ref_frames_end_idx =
          cm->features.tip_frame_mode ? EXTREF_FRAME : REF_FRAMES;
      for (int i = 0; i < ref_frames_end_idx; i++)
#else
  ...
  // write deltas to frame buffer
#if CONFIG_TIP
  memcpy(cm->cur_frame->ref_deltas, lf->ref_deltas, EXTREF_FRAME);
#else
  ...
  if (current_frame->frame_type == KEY_FRAME) {
    ...
    if (!(current_frame->frame_type == INTRA_ONLY_FRAME) &&
      ...
      if (cm->seq_params.enable_tip) {
        ...
        if (features->tip_frame_mode >= TIP_FRAME_MAX_MODE) {
          aom_internal_error(&cm->error, AOM_CODEC_CORRUPT_FRAME,
                             "Invalid TIP mode.");
        }
...
static AOM_INLINE void process_tip_mode(AV1Decoder *pbi) {
  ...
  if (cm->features.allow_ref_frame_mvs && cm->has_bwd_ref) {
    if (cm->features.tip_frame_mode == TIP_FRAME_AS_OUTPUT) {
      ...
      av1_setup_tip_motion_field(cm, 0);
      const int mvs_rows =
          ROUND_POWER_OF_TWO(cm->mi_params.mi_rows, SHIFT_BITS);
      const int mvs_cols =
