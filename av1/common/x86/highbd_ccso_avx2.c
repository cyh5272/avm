...
void ccso_filter_block_hbd_wo_buf_avx2(const uint16_t* src_y, uint16_t* dts_yuv, const int x, const int y, const int pic_width, const int pic_height, int* rec_luma_idx, const int8_t* offset_buf,
  ...
  const int y_uv_hscale, const int y_uv_vscale, 
    ...
    const int quant_step_size, const int inv_quant_step, const int* rec_idx, const int max_val, const int blk_size, const bool isSingleBand, const uint8_t shift_bits) {
  ...
  for (int yOff = 0; yOff < y_offset; yOff++)
  {
    ...
    //const uint16_t* src_rec2 = src_y + ((src_y_stride[yOff] << y_uv_vscale) + (x << y_uv_hscale)) + pad_stride;
    const uint16_t *src_rec2 = src_y + ((yOff << y_uv_vscale) * src_y_stride + (x << y_uv_hscale));

    //int stride = src_y_stride[yOff] << y_uv_vscale;
    for (int xOff = 0; xOff < x_offset; xOff += 16)
    {
      //uint16_t* rec_tmp = &src_rec2[xOff << y_uv_hscale];
      __m256i rec_curlo = _mm256_loadu_si256((const __m256i*)(src_rec2 + (xOff << y_uv_hscale)));
      __m256i rec_cur_final;

      //__m256i rec_tap1 = _mm256_loadu_si256((const __m256i*)(src_rec2 + (xOff << y_uv_hscale) + tap1_pos));
      __m256i rec_tap1lo = _mm256_loadu_si256((const __m256i*)(src_rec2 + (xOff << y_uv_hscale) + tap1_pos));
      //__m256i rec_tap2 = _mm256_loadu_si256((const __m256i*)(src_rec2 + (xOff << y_uv_hscale) + tap2_pos));
      __m256i rec_tap2lo = _mm256_loadu_si256((const __m256i*)(src_rec2 + (xOff << y_uv_hscale) + tap2_pos));

      if (y_uv_hscale > 0) {
        __m256i rec_curhi = _mm256_loadu_si256((const __m256i*)(src_rec2 + (xOff << y_uv_hscale) + 16));
      ...
      __m256i num_band = isSingleBand? all0: _mm256_srli_epi16(rec_cur_final, shift_bits);
      ...
      //uint16_t val = clamp(offset_val + dst_rec2[xOff], 0, (1 << cm->seq_params.bit_depth) - 1);
    ...
    for (int xOff = x_offset; xOff < x_offset + x_remainder; xOff++) {
      //cal_filter_support(rec_luma_idx, &src_y[((src_y_stride[yOff] << y_uv_vscale) + ((x + xOff) << y_uv_hscale)) + pad_stride], quant_step_size, inv_quant_step, rec_idx);
      cal_filter_support(rec_luma_idx, &src_y[((yOff << y_uv_vscale) * src_y_stride + ((x + xOff) << y_uv_hscale)) ], quant_step_size, inv_quant_step, rec_idx);
      const int band_num = isSingleBand ? 0: src_y[((yOff << y_uv_vscale) * src_y_stride + ((x + xOff) << y_uv_hscale))] >> shift_bits;
      int offset_val = offset_buf[(band_num <<4) + (rec_luma_idx[0] << 2) + rec_luma_idx[1]];
      //dts_yuv[dst_stride[yOff] + x + xOff] = clamp(offset_val + dts_yuv[dst_stride[yOff] + x + xOff], 0, max_val);
      dts_yuv[ yOff * dst_stride + x + xOff] = clamp(offset_val + dts_yuv[ yOff * dst_stride + x + xOff], 0, max_val);
    }
  }
}
  ...
  for (int yOff = 0; yOff < y_offset; yOff++) {
    ...
    for (int xOff = 0; xOff < x_offset; xOff += 16) {
      ...
      if (y_uv_hscale > 0) {
        ...
        _mm_storeu_si128((__m128i *)(src_cls0_2 + (xOff<< y_uv_hscale)), idx1_128lo);
        _mm_storeu_si128((__m128i *)(src_cls1_2 + (xOff<< y_uv_hscale)), idx2_128lo);
        _mm_storeu_si128((__m128i *)(src_cls0_2 + (xOff<< y_uv_hscale) + 16), idx1_128hi);
        _mm_storeu_si128((__m128i *)(src_cls1_2 + (xOff<< y_uv_hscale) + 16), idx2_128hi);
      } else {
        //src_cls0[(y_pos << y_uv_vscale) * ccso_stride + (x_pos << y_uv_hscale)] = src_cls[0];
    ...
    for (int xOff = x_offset; xOff < x_offset + x_remainder; xOff++) {
      ...
      cal_filter_support(src_cls,  &src_y[((yOff << y_uv_vscale) * src_y_stride + ((x + xOff) << y_uv_hscale))],
                         quant_step_size, inv_quant_step, src_loc);
      src_cls0[(yOff << y_uv_vscale) * ccso_stride + ((x + xOff) << y_uv_hscale)] = src_cls[0];
      src_cls1[(yOff << y_uv_vscale) * ccso_stride + ((x + xOff) << y_uv_hscale)] = src_cls[1];
...
void ccso_filter_block_hbd_with_buf_avx2(
    const uint16_t *src_y, uint16_t *dts_yuv, const uint8_t *src_cls0, const uint8_t *src_cls1, 
    const int src_y_stride, const int dst_stride, 
  ...
  for (int yOff = 0; yOff < y_offset; yOff++) {
    ...
    for (int xOff = 0; xOff < x_offset; xOff += 16) {
      // uint16_t* rec_tmp = &src_rec2[xOff << y_uv_hscale];
      __m256i rec_curlo = _mm256_loadu_si256((const __m256i *)(src_rec2 + (xOff << y_uv_hscale)));
      __m128i cur_src_cls0lo =_mm_lddqu_si128((const __m128i *)(src_cls0_2 + (xOff << y_uv_hscale)));
      __m128i cur_src_cls1lo =_mm_lddqu_si128((const __m128i *)(src_cls1_2 + (xOff << y_uv_hscale)));
      __m256i rec_cur_final;
    ...
    for (int xOff = x_offset; xOff < x_offset + x_remainder; xOff++) {
      ...
      // y_uv_vscale) + ((x + xOff) << y_uv_hscale)) + pad_stride],
      // quant_step_size, inv_quant_step, rec_idx);
      int cur_src_cls0 = src_cls0[(yOff << y_uv_vscale) * ccso_stride +  ((x + xOff) << y_uv_hscale)];
      int cur_src_cls1 = src_cls1[(yOff << y_uv_vscale) * ccso_stride +  ((x + xOff) << y_uv_hscale)];
      const int band_num = src_y[((yOff << y_uv_vscale) * src_y_stride + ((x + xOff) << y_uv_hscale))] >> shift_bits;
      int offset_val = filter_offset[(band_num << 4) + (cur_src_cls0 << 2) + cur_src_cls1];
...
uint64_t compute_distortion_block_avx2(const uint16_t *org, const int org_stride,
