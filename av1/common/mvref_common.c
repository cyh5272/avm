    ...
    if (candidate->ref_frame[0] == rf[0] && candidate->ref_frame[1] == rf[1]) {
      ...
      if (candidate_ref_idx0 != -1 && candidate_ref_idx1 != -1) {
        ...
        if (cand_idx == *single_mv_count &&
          ...
          single_mv[cand_idx].mv.as_int = this_refmv[candidate_ref_idx0].as_int;
...
static AOM_INLINE bool fill_ref_bv_list(CANDIDATE_MV cand_mv,
...
static AOM_INLINE void setup_ref_mv_list(
  ...
  *refmv_count = 0;
  ...
  if (xd->mi[0]->use_intrabc[xd->tree_type == CHROMA_PART]) {
    ...
    int default_ref_bv_list[MAX_REF_BV_STACK_SIZE][2] = {
      { 0, -128 },
    ...
    for (int i = 0; i < MAX_REF_BV_STACK_SIZE; i++) {
      ...
      fill_ref_bv_list(tmp_mv, ref_mv_stack, ref_mv_weight, refmv_count);
    }
    assert(*refmv_count >= MAX_REF_BV_STACK_SIZE);
  }
#endif  // CONFIG_BVP_IMPROVEMENT
}
