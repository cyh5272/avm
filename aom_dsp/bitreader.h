...
#define BYPASS_WEIGHT 0.25  // weighting for bypass coded symbols
#endif
...
#if CONFIG_THROUGHPUT_ANALYSIS
    if (is_context_coded && coding_mode == SYMBOL_CDF) {
      r->accounting->syms.num_ctx_coded += n_bits;
      if (r->accounting->syms.counter_bypass_string) {
        r->accounting->syms.cycles_bypass_coded += (int)ceil(
            r->accounting->syms.counter_bypass_string * BYPASS_WEIGHT);
      }
      r->accounting->syms.counter_bypass_string = 0;
    } else {
      r->accounting->syms.num_bypass_coded += n_bits;
      r->accounting->syms.counter_bypass_string += n_bits;
    }
...
#if CONFIG_THROUGHPUT_ANALYSIS
  aom_update_symb_counts(r, 1, 0, 1, NULL, SYMBOL_BIT);
#else
...
#if CONFIG_THROUGHPUT_ANALYSIS
  aom_update_symb_counts(r, 1, 0, 1, NULL, SYMBOL_BIT_BYPASS);
#else
  aom_update_symb_counts(r, 1, 1);
#endif
#endif
...
#if CONFIG_THROUGHPUT_ANALYSIS
  aom_update_symb_counts(r, 1, 0, bits, NULL, SYMBOL_LITERAL_BYPASS);
#else
...
#if CONFIG_THROUGHPUT_ANALYSIS
  aom_update_symb_counts(r, 1, 0, n_bits, NULL, SYMBOL_UNARY);
#else
...
#if CONFIG_THROUGHPUT_ANALYSIS
  aom_update_symb_counts(r, (nsymbs == 2), 1, 1, cdf, SYMBOL_CDF);
#else
